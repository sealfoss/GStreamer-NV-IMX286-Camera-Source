...
/* Compile-time tuning paths from CMake: the installed share/ location
 * (the property default) and the source tree's data/ copy (fallback for
 * running from a build directory before `make install`). */
#ifndef NVIMX296_DEFAULT_TUNING
#define NVIMX296_DEFAULT_TUNING "/usr/local/share/nvimx296camerasrc/imx296_16mm.json"
#endif
#ifndef NVIMX296_SOURCE_TUNING
#define NVIMX296_SOURCE_TUNING ""
#endif
...
static gboolean nvimx296_start(GstBaseSrc *bsrc)
{
    ...
    if (isp_tuning_load(tf, &self->tuning) == 0)
        return (self->isp_params.awb_ct =
                    self->isp_params.awb_ct ? self->isp_params.awb_ct : 4560.0), TRUE;

    /* only if the user did NOT override the property, try the source-tree
     * copy (uninstalled/dev runs from a build directory) */
    if (!g_strcmp0(tf, NVIMX296_DEFAULT_TUNING) && NVIMX296_SOURCE_TUNING[0] &&
        isp_tuning_load(NVIMX296_SOURCE_TUNING, &self->tuning) == 0) {
        GST_INFO_OBJECT(self, "using source-tree tuning %s (not installed yet)",
                        NVIMX296_SOURCE_TUNING);
        self->isp_params.awb_ct =
            self->isp_params.awb_ct ? self->isp_params.awb_ct : 4560.0;
        return TRUE;
    }

    GST_ELEMENT_ERROR(self, RESOURCE, NOT_FOUND,
        ("tuning file unusable: %s%s%s%s - run `sudo make install` or pass "
         "tuning-file=", tf,
         NVIMX296_SOURCE_TUNING[0] ? " (also tried " : "",
         NVIMX296_SOURCE_TUNING[0] ? NVIMX296_SOURCE_TUNING : "",
         NVIMX296_SOURCE_TUNING[0] ? ")" : ""), (NULL));
    return FALSE;
}
...
static void gst_nvimx296camerasrc_init(GstNvImx296CameraSrc *self)
{
    ...
    self->tuning_file = g_strdup(NVIMX296_DEFAULT_TUNING);
